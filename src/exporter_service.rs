...
impl ExporterService {
    ...
    pub async fn run(&self, start_date: DateTime<Utc>) -> Result<(), Box<dyn Error>> {
        ...
        let mut retrieved_spot_prices = clean_spot_prices(
            local_time_zone,
...
fn clean_spot_prices(local_time_zone: Tz, spot_prices: Vec<SpotPrice>) -> Vec<SpotPrice> {
    if spot_prices.len() == 25 {
...
mod tests {
    ...
    #[test]
    fn clean_spot_prices_returns_correct_times_for_switch_to_summer_time(
        ...
        // act
        let cleaned_spot_prices = clean_spot_prices(
            local_time_zone,
            spot_price_response.data.market_prices_electricity,
        );

        assert_eq!(cleaned_spot_prices.len(), 23);

        assert_eq!(
            cleaned_spot_prices[0].from,
            Utc.ymd(2022, 3, 26).and_hms(23, 0, 0)
        );
        assert_eq!(
            cleaned_spot_prices[0].till,
            Utc.ymd(2022, 3, 27).and_hms(0, 0, 0)
        );

        assert_eq!(
            cleaned_spot_prices[1].from,
            Utc.ymd(2022, 3, 27).and_hms(0, 0, 0)
        );
        assert_eq!(
            cleaned_spot_prices[1].till,
            Utc.ymd(2022, 3, 27).and_hms(1, 0, 0)
        );

        assert_eq!(
            cleaned_spot_prices[2].from,
            Utc.ymd(2022, 3, 27).and_hms(1, 0, 0)
        );
        assert_eq!(
            cleaned_spot_prices[2].till,
            Utc.ymd(2022, 3, 27).and_hms(2, 0, 0)
        );

        assert_eq!(
            cleaned_spot_prices[22].from,
            Utc.ymd(2022, 3, 27).and_hms(21, 0, 0)
        );
        assert_eq!(
            cleaned_spot_prices[22].till,
    ...
    #[test]
    fn clean_spot_prices_returns_correct_times_for_switch_to_winter_time(
    ) -> Result<(), Box<dyn Error>> {
        let response_body = r#"{"data":{"marketPricesElectricity":[
        ...
        assert_eq!(cleaned_spot_prices.len(), 25);
        assert_eq!(
            cleaned_spot_prices[0].from,
        ...
        assert_eq!(
            cleaned_spot_prices[24].from,
            Utc.ymd(2021, 10, 31).and_hms(22, 0, 0)
        );
        assert_eq!(
            cleaned_spot_prices[24].till,
    ...
    #[test]
    fn clean_spot_prices_returns_winter_time_as_is() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(cleaned_spot_prices.len(), 24);
        assert_eq!(
            cleaned_spot_prices[0].from,
        ...
        assert_eq!(
            cleaned_spot_prices[23].from,
            Utc.ymd(2022, 1, 1).and_hms(22, 0, 0)
        );
        assert_eq!(
            cleaned_spot_prices[23].till,
    ...
    #[test]
    fn clean_spot_prices_returns_summer_time_shifted_one_hour() -> Result<(), Box<dyn Error>> {
